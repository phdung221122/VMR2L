...
def parse_args():
    ...
    parser.add_argument("--restore-name", type=str, default=None, help="restore experiment name")
    parser.add_argument("--restore-id", type=str, default=None, help="restore wandb id")
    parser.add_argument("--pretrain", action='store_true',
                        help="if toggled, we will restore pretrained weights for vm selection")
    parser.add_argument("--gym-id", type=str, default="generalizer-v2",
    ...
    parser.add_argument("--affinity", type=int, default=-1, help="number of conflicts per VM")
...
def make_env(gym_id, seed, vm_data_size, max_steps, num_train, normalize, affinity):
    def thunk():
        if 'affinity' in gym_id:
            env = gym.make(gym_id, seed=seed, vm_data_size=vm_data_size, max_steps=max_steps, train_range=num_train,
                           normalize=normalize, affinity=affinity)
        else:
            env = gym.make(gym_id, seed=seed, vm_data_size=vm_data_size, max_steps=max_steps, train_range=num_train,
                           normalize=normalize)
...
if __name__ == "__main__":
    ...
    if args.model == 'attn':
        ...
        # pm_cand_model = models.PM_Attn_No_Flip_Wrapper(params).model
    elif args.model == 'mlp':
    ...
    if args.restore_name is not None:
        checkpoint = torch.load(f'./runs/{args.restore_name}/latest.pth.tar')
        agent.load_state_dict(checkpoint['state_dict'])
        optim.load_state_dict(checkpoint['optim_dict'])
        global_step = checkpoint['global_step']
        start_epoch = checkpoint['global_step'] // args.num_envs // num_steps
        print(f'Resume at step {start_epoch}...')
    else:
        start_epoch = 1

    num_updates = args.total_timesteps // args.batch_size
    pbar = trange(start_epoch, num_updates + 1)
    reset_env = False
    for update in pbar:
        ...
        if args.track:
            ...
            writer.add_scalar("episode_details/min_fragment_rate", np.mean(np.amin(current_ep_info[:, :, 1], axis=0)),
                              global_step)
        pbar.set_description(f'Train frag rate: {np.mean(np.amin(current_ep_info[:, :, 1], axis=0)):.4f}')
        # if args.track:
